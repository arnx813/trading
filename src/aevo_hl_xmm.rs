...
impl XMM {
    pub async fn new(
        ...
        XMM {
            aevo_state: AevoState {
                ...
                buy_open_sz: 0.0,
                sell_open_sz: 0.0,
                bid_resting_order: RestingOrder {
                    px: -1.0,
                    sz: 0.0,
                    order_id: "".to_string(),
                    status: AevoOrderStatus::Cancelled,
                    filled: 0.0,
                },
                ask_resting_order: RestingOrder {
                    px: -1.0,
                    sz: 0.0,
                    order_id: "".to_string(),
                    status: AevoOrderStatus::Cancelled,
        ...
        self.hl_info_client
            .subscribe(
                Subscription::UserEvents {
                    user: self.hl_exchange_client.wallet.address(),
                },
                hl_tx.clone(),
            )
            .await
    ...
    async fn process_aevo_tickers(&mut self, ticker: &BookTicker) {
        ...
        let new_resting_order: Result<RestingOrder> = match resting_order.status {
            AevoOrderStatus::Opened | AevoOrderStatus::Partial => self
                .aevo_client
                .rest_edit_order(
            ...
            AevoOrderStatus::Filled => Err(eyre!("Order Filled Already")),
            _ => unreachable!(),
        };
    ...
    async fn process_aevo_fills(&mut self, fill: Fill) {
        ...
        let order = if aevo_is_buy {
            &mut aevo_state.bid_resting_order
        } else {
            &mut aevo_state.ask_resting_order
        };
        ...
        match order_status {
            AevoOrderStatus::Filled | AevoOrderStatus::Partial => {
                ...
                let hl_order_amount = if hl_is_buy {
                    aevo_state.buy_open_sz - hl_state.sell_open_sz
                } else {
                    aevo_state.sell_open_sz - hl_state.buy_open_sz
                };
                    ...
                    let response = self
                        ...
                        .order(
                            ClientOrderRequest {
                                ...
                                is_buy: hl_is_buy,
                                reduce_only: false,
                                limit_px: if hl_is_buy {
                                    hl_state.ask_px * 1.05
                                } else {
                                    hl_state.bid_px * 0.95
                                }, // 5 % max slippage
                                sz: hl_order_amount,
                                cloid: None,
                                    ...
                                    if !order.statuses.is_empty() {
                                        match &order.statuses[0] {
                                            ExchangeDataStatus::Filled(order) => {
                                                if hl_is_buy {
                                                    hl_state.buy_open_sz = hl_state.buy_open_sz
                                                        + order.total_sz.parse::<f64>().unwrap();
                                                    info!("Market Buy Order Filled with size : {} and avg price : {}", order.total_sz, order.avg_px);
                                                    panic!();
                                                } else {
                                                    hl_state.sell_open_sz = hl_state.sell_open_sz
                                                        + order.total_sz.parse::<f64>().unwrap();
