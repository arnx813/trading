...
impl XArb {
    ...
    pub async fn start(&mut self) {
        ...
        loop { 
            println!("Aevo State : {:?}, HL State : {:?}", self.aevo_state, self.hl_state); 
            tokio::select! {
                            ...
                            if !order.statuses.is_empty() {
                                match &order.statuses[0] {
                                    ExchangeDataStatus::Filled(order) => {
                                        ...
                                        self.hl_state.sell_open_sz = self.hl_state.sell_open_sz + amount_filled;
                                        self.hl_state.buy_open_sz = self.hl_state.buy_open_sz - amount_filled; 
                                ...
                                panic!("Exchange data statuses is empty when placing order: {order:?}")
                            }
                        } else {
